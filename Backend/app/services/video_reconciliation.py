"""Reconciliação manual/admin de reservas de vídeo, sem agendador.

Execute reconcile_video_reservations() e passe next_cursor na próxima chamada
...
Query: type == video-generation, creditStatus == reserved, ordenada por ID.
...
Não há TTL: duração de vídeo (até 15s), timeout HTTP e polling de 5s não são
limites de processamento do provedor. Sem taskId, é necessária reconsulta por
...
from app.services.deepswap import get_video_task
...
def list_reserved_video_generations(limit: int = 100, after_id: str | None = None) -> list:
    ...
    query = (
        collection.where(filter=FieldFilter("type", "==", "video-generation"))
...
async def reconcile_video_reservations(limit: int = 100, after_id: str | None = None) -> dict:
    """Processa uma página. Não confiar em estados de sucesso enviados por clientes."""
    snapshots = list_reserved_video_generations(limit, after_id)
    results = []
    for snapshot in snapshots:
        ...
        try:
            ...
            if generation.get("type") != "video-generation" or generation.get("creditStatus") != "reserved":
                outcome = "skipped"
            else:
                ...
                if task_id is None:
                    ...
                    task = await get_video_task(task_id, log_response=False)
